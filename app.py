    ...
    def setup_routes(self):
        ...
        def analyze():
            try:
                ...
                query = data.get('query', 'Analyze this repository structure and functionality')
        ...
        def github_webhook():
            try:
                ...
                if self.config.get("github_webhook_secret") and signature:
                ...
                if payload.get('ref') == f'refs/heads/{self.config["repositories"][repo_name]["branch"]}':
                    ...
                    if commits:
                        ...
                        if modified_files:
                            query = f"Review the recent changes in these files: {', '.join(modified_files[:5])}"
                            file_contents = self.fetch_repo_via_github_api(repo_name)
